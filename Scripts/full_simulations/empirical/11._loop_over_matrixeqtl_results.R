...
##   matrixEQTL/rep_sims/n100, matrixEQTL/rep_sims/n500, etc
...
## Cap the number of results files for each sample size
cis_files <- lapply(
  cis_files,
  function(x) x[1:min(30, length(x))]
)
...
power.agg <- power.dt[, list(
    ...
    power_filtered_fdr_lwr = quantile(power_filtered_fdr, probs = 0.025),
    power_filtered_fdr_upr = quantile(power_filtered_fdr, probs = 0.975),
    power_all_fdr          = mean(power_all_fdr),
    power_all_fdr2         = mean(power_all_fdr2),
    power_filtered_by      = mean(power_filtered_by),
    power_filtered_bon     = mean(power_filtered_bon),
    power_all_bon          = mean(power_all_bon)
  ),
  by = c("n", "es_bin", "maf_bin", "qtl_tag")
]

power.agg$sample_size <- as.numeric(as.character(power.agg$n))

power.agg <- power.agg[order(qtl_tag, sample_size, es_bin)]

fwrite(power.agg, file = "Output/power/matrixEQTL.csv")
