...
sa_power_calc <- function(adjusted_matrix, method_name) {
  ...
  ## given realisation
  power_dt <- data.table(
    sa_curr_n_power[, c("es_bin", "qtl_tag", "cis_flag")],
  ...
  setkey(power_dt, qtl_tag, es_bin, cis_flag)
...
for (curr_n in sample_sizes) {
  ...
  rm(p_mat)
  rm(f_mat_all, f_mat_filt, b_mat_filt, b_mat_all, by_mat_filt, by_mat_all)
...
sa_power_all_averaged <- sa_power_all[order(sample_size, es_bin)]

fwrite(
  sa_power_all_averaged,
  file = sprintf("Output/power/sa_mvn%s.csv", ifelse(ancestry != "", ancestry, "_eur"))
)
