...
#### Nullified genes
# Run sa_multiple_sample_sizes_mvn up to null genes
sa_results_null <- readRDS("Output/semi_analytic/sa_results_mvn_nullgenes_random.rds")
message(nrow(sa_results_big), ", ", nrow(sa_results_null))
sa_results_null <- sa_results_null[pid %in% null_genes]
sa_results_big  <- sa_results_big[!(pid %in% null_genes)]
message(nrow(sa_results_big), ", ", nrow(sa_results_null))
sa_results_big <- rbind(sa_results_big, sa_results_null, fill = TRUE)
message(nrow(sa_results_big))
results_merge <- results_merge[cis_flag == TRUE]
...
for (curr_n in sample_sizes) {
  ...
  sa_curr_n_power_genes <- sa_results_big_n[, 1:10][, list(
    ...
    # egene_justin = pid %in% justin_sig_genes
  ),
  by = "pid"
  ]
...
fwrite(sa_power_all_averaged, file = "Output/power_egenes/sa_mvn_nullgenes_random.csv")
