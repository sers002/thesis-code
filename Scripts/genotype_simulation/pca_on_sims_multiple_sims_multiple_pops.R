...
original.vcf <- fread("./Output/genotype_simulation/1000g_chr19_for_pca.vcf.gz", skip = 100)

original.vcf <- original.vcf[ID %in% hapgen.dip1$snp]
original.vcf <- original.vcf[!duplicated(ID)]

## VCFs have "0|0" for the genotype for each participant, so turn these into 0,1,2
pb <- txtProgressBar(min = 10, max = ncol(original.vcf), style = 3)

for (i in 10:ncol(original.vcf)) {
  setTxtProgressBar(pb, i)
  
  curr_col <- names(original.vcf)[i]
  set(
    original.vcf, 
    j = curr_col, 
    value = as.numeric(substr(original.vcf[[curr_col]], 1, 1)) + as.numeric(substr(original.vcf[[curr_col]], 3, 3))
  )
}
...
original.mat <- sparsify(original.vcf[, 10:ncol(original.vcf)])

original.mat <- sparsify(original.vcf[, 10:500])
original.mat <- cbind(original.mat, sparsify(original.vcf[, 501:1000]))
original.mat <- cbind(original.mat, sparsify(original.vcf[, 1001:1500]))
original.mat <- cbind(original.mat, sparsify(original.vcf[, 1501:2000]))
original.mat <- cbind(original.mat, sparsify(original.vcf[, 2001:ncol(original.vcf)]))
